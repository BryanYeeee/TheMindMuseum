...
function ViewerObject({ model_url }) {
    const ref = useRef();
    const { scene } = model_url
        ? useGLTF(`http://localhost:5001${model_url}`)
        : { scene: null };
    useFrame((_, delta) => {
...
function ViewerPainting({ image_url }) {
            ...
            {exhibit && (
                <motion.div
                    ...
                    <div className="w-1/2 h-full">
                        <Canvas camera={{ position: [0, 0, 4], fov: 50 }}>
                            ...
                            {exhibit.type === "painting" ? (
                                <ViewerPainting image_url={exhibit.image_url} />
                            ) : (
                    ...
                    <motion.div
                        ...
                        <p className="text-amber-500 text-[9px] tracking-[0.3em] uppercase mb-5">
                            Exhibit
                        </p>
